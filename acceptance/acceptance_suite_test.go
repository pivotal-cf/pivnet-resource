...
var _ = SynchronizedBeforeSuite(func() []byte {
	...
	suiteEnv.CheckPath, err = gexec.Build("github.com/pivotal-cf/pivnet-resource/v3/cmd/check", "-race")
	Expect(err).NotTo(HaveOccurred())

	By("Compiling out binary")
	suiteEnv.OutPath, err = gexec.Build("github.com/pivotal-cf/pivnet-resource/v3/cmd/out", "-race")
	Expect(err).NotTo(HaveOccurred())

	By("Compiling in binary")
	suiteEnv.InPath, err = gexec.Build("github.com/pivotal-cf/pivnet-resource/v3/cmd/in")
	Expect(err).NotTo(HaveOccurred())
